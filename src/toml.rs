...
fn check_version(config: &Config) {
    ...
    assert!(
        req.matches(&version),
        "incompatible versions of `hs-pack`/`hs-bindgen` used, please update"
    );
}
